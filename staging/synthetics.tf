...
# ===============================================================================
# Amazon CloudWatch Synthetics Canary for Check Response API Monitoring
# ===============================================================================
resource "aws_synthetics_canary" "check_response_api" {
  name                     = "${local.project}-${local.env}-cwt-syn-check-response-api"
  artifact_s3_location     = aws_s3_bucket.synthetics_artifacts.arn
  execution_role_arn       = aws_iam_role.cloudwatch_synthetics.arn
  handler                  = "function.canary_handler"
  runtime_version          = "syn-python-selenium-11.1"
  zip_file                 = data.archive_file.canary_check_response_api.output_path
  success_retention_period = 31
  failure_retention_period = 31
...
data "archive_file" "canary_check_response_api" {
  type        = "zip"
  source_dir  = "${path.cwd}/files/canary/canary-check-response-api"
  output_path = "${path.module}/artifacts/canary-check-response-api.zip"
}
